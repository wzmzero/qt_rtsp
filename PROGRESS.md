...
## 2026-03-06（紧急UI重构 + 告警规则改版）

### 已完成
1. **实时页布局按最新要求重排**
   - 左：视频主区域（主宽度）
   - 右：纵向三块（连接状态 / 数据接收 / 参数编辑）

2. **连接状态块精简**
   - 仅保留 3 个状态灯：RTSP 是否连接、TCP 是否连接、是否响应绑定。
   - 移除长连接字符串展示。

3. **数据接收块增强**
   - 上半区：原始数据摘要（最近N条 + 单条截断）+ “查看完整”弹窗。
   - 下半区：解析结果突出告警级别、触发条件（person_conf/rod_conf）、经纬度与检测摘要 TopN。

4. **告警逻辑切换（不再使用IoU）**
   - 改为同消息 person+rod 且各自置信度过阈值触发。
   - 三档告警颜色：低=浅黄 / 中=深黄 / 高=红。
   - 阈值参数可配置并持久化（默认 low=0.50, mid=0.70, high=0.85）。

5. **主题绑定与状态同步修复**
   - 主题菜单勾选与当前主题双向绑定。
   - 保存配置后保持当前主题，不再回退暗色。
   - 连接灯/告警灯统一通过刷新入口更新（`refreshConnectionUi` / `refreshParsedUi`）。

6. **联调验证**
   - 构建：`cmake -S . -B build && cmake --build build -j4` ✅
   - 无头联调：`sim_server --tcp-port 19090` + `QT_CLIENT_AUTOTEST=1 qt_client` ✅
   - 日志证据包含三档告警切换：`low -> mid -> high` ✅

### 已知限制
- 无头环境仅能做基础交互与持久化验证；完整视觉布局与菜单勾选行为仍建议在桌面环境做一次人工确认。

## 2026-03-06（UI规范修正：主界面整合 + 全局菜单）

### 本轮目标
- 取消“数据视图独立页”，将接收数据与解析结果回归主界面主区域。
- 菜单栏固定为 文件 / 页面 / 功能 / 帮助 四项，且保存配置与主题切换归入菜单全局动作。
- 优化主界面布局比例与可读性。
- 构建验证并更新文档。
...
1. **页面结构调整**
   - 移除“数据视图”独立页。
   - 实时页改为主工作台：视频、连接/告警、参数区、接收数据、解析结果同页可见。
   - 页面菜单改为 `实时 / 回放 / 日志 / 事件` 四页切换。
...
3. **全局动作收口**
   - 参数区删除“保存配置”与主题控件，仅保留启动/停止和参数输入。
   - 主题切换改为菜单动作并即时生效；保存配置通过文件菜单统一触发。

4. **布局优化**
   - 实时页采用“左主右辅”比例：左侧（视频+连接告警）: 右侧（参数+接收数据+解析结果）≈ 3:2。
   - 调整间距、最小窗口尺寸、文本区高度，提升信息密度下的阅读体验。
